...
class TableExtractor:
    ...
    def find_largest_contour_by_area(self):
        ...
        max_areas = [0, 0, 0]
        contours_with_max_areas = [None, None, None]
        for contour in self.rectangular_contours:
            area = cv2.contourArea(contour)
        
            # Compare the area with the three largest areas
            for i in range(3):
                if area > max_areas[i]:
                    # If the current area is larger, update the list of three largest areas and contours
                    max_areas[i+1:] = max_areas[i:-1]
                    max_areas[i] = area
                    contours_with_max_areas[i+1:] = contours_with_max_areas[i:-1]
    ...
    def order_points_in_the_contour_with_max_area(self):
        ...
        for point in self.contour_with_max_area_ordered:
            ...
            self.image_with_points_plotted = cv2.circle(self.image_with_points_plotted, point_coordinates, 20, (0, 0, 255), -1)
